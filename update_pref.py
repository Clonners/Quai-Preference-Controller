...
async def process_block(hdr, session, state):
    ...
    """
    # 1) Extraer número de bloque y dificultad real, soportando:
    #    - wrapper interno de Quai: hdr["woHeader"]
    #    - JSON-RPC estándar: hdr["number"], hdr["difficulty"]
    if "woHeader" in hdr:
        w = hdr["woHeader"]
        blk_hex, diff_hex = w.get("number"), w.get("difficulty")
    else:
        blk_hex, diff_hex = hdr.get("number"), hdr.get("difficulty")
    ...
    # 3) Actualizar k_Quai según EMA de log2(d_star)
    state["k_quai"] += ALPHA_RATE * ((math.log2(d_star) if d_star>0 else 0) - state["k_quai"])

    # 4) Calcular k_Qi con “doubling schedule”
    ...
    # 8) Convierte QiWei a Wei de Quai
            ...
            try:
                ...
                async with websockets.connect(RPC_WS) as ws:
                    ...
                    async for raw in ws:
                        ...
                        # procesar si viene wrapper interno o estándar
                        if "woHeader" in hdr or ("number" in hdr and "difficulty" in hdr):
